...
with open('hex_md5.js', mode='r', encoding='utf-8') as f:
    js_code = f.read()
...
cookies = {
    'sessionid': '9o7kc7z5ekhaubrl77pzp6mf9v8m8wgc'
}
...
def main(data):
    global sum_num, sum_count, no_display_code
    ...
    for td in tds:
        ...
        imgs = td.xpath('.//img')
        for img in imgs:
            img_number = img.xpath('.//@class')[0]
            if img_number.split(' ')[1] == no_display_code:
                continue
            else:
                i += 1
                number.append(i)
        ...
        for img in imgs:
            ...
            if img_number.split(' ')[1] == no_display_code:
                continue
            number_img = img.xpath('.//@src')[0]
            iv = img.xpath('.//@style')[0][5:-2]
            if iv == '0.0' or iv == '0':
                iv = 0
            elif iv == '11.5':
                iv = 1
                ...
                iv = -3
            # number.insert(i+iv, number_img_data[number_img])
            number[i+iv] = number_img_data[number_img]
            print(number, number_img_data[number_img], iv, i)
            i += 1
        number.reverse()
        print(number)
        i = 1
...
for i in range(1, 6):
    ...
    resp = requests.get(url, headers=headers, cookies=cookies, params=params).json()
    no_display_code = js.call('main', resp['key'], resp['value'])
    main(resp['info'])
