...
+ task用法实例
  ...
  async def func(sleep_time):
      # 使用 sleep_time (函数 I/O 等待时长)作为函数名后缀，以区分任务对象
...
我们可以读取到服务器的响应状态和响应内容，这也是使用请求的一个很重要的部分。通过`status`来获取响应状态码，`text()`来获取到响应内容，当然也可以直接指明编码格式为你想要的编码格式：
            ...
            async with signal:
                async with aiohttp.ClientSession() as session:
                    async with session.get(chapter_url) as resp:
                        ...
                        chapter_name = chapter_name.replace(" ", "").replace(":", "").strip()
